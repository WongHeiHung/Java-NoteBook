...
public class ParallelMerger {
    ...
    private static class Worker implements Runnable {
        ...
        private int index; // Worker index to determine order

        public Worker(String segment, int index) {
        ...
        public void run() {
            ...
            for (int i = 0; i < length; i++) {
                synchronized (lock) {
                    while (currentWorker % totalWorker != index) {
                        try {
                            lock.wait();
                    ...
                    resultWriter.write(segment.charAt(i));
                    currentWorker++;
                    lock.notifyAll();
                }
            }
    ...
    public static void merge(String[] segments, ThreadSafeCharacterWriter resultWriter) throws InterruptedException {
        ...
        // Start from here
        // ...
        totalWorker = segments.length;
        Thread[] threads = new Thread[totalWorker];

        for (int i = 0; i < totalWorker; i++) {
            Worker worker = new Worker(segments[i], i);
